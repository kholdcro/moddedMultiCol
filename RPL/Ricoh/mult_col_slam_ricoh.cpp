...
int main(int argc, char **argv)
{
	...
	for (int ni = 0; ni < nImages; ni++)
	{
		...
		trackState[ni] = MultiSLAM.GetTrackingState();
	...
	cout << "Map Points: " << MultiSLAM.GetTotalMapPoints() << endl;

	// Save camera trajectory
	MultiSLAM.SaveMKFTrajectoryLAFIDA(path2imgs+"MKFTrajectory.txt");

    ofstream g;
    g.open((path2imgs+"/stateFrames.txt").c_str());
    for(int ni=0; ni<nImages; ni++)
    {
	...
	ofstream f;
	    f.open((path2imgs+"/trackPercentage.txt").c_str());
	    f << fixed;
	    f << "median tracking time: " << vTimesTrack[nImages/2] << "\n" << endl;
	    f << "mean tracking time: " << totaltime/nImages << "\n" << endl;
	    f << "Average tracking time: " << totaltime/timestamps[nImages-1] << "\n" << endl;
	    f << "Total tracking time: " << totaltime << "\n" << endl;
	    f << "Total time: " << timestamps[nImages-1] << "\n" << endl;
	    f << "Number of images: " << nImages << "\n" << endl;
	    f << "Total Map Points: " << MultiSLAM.GetTotalMapPoints() << "\n" << endl; 
	    f << "-------";
    f.close();
